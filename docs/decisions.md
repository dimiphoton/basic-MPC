...
| 2026-09-11 | Contrôle v1.1 : consigne \(T_{\mathrm{sp}}\), bande \(n=1\) °C, \(J\) = facture HP/HC + inconfort ; pas de PV dans l'euro | Commander \(P\) ; fioul ; PV dans la facture ; trois poids SciPy | Un thermostat reçoit une consigne. L'euro et le calendrier se lisent. Le PV dans \(J\) imposerait des prévisions de production. Détail : `brief/controle-mpc.md`. |
| 2026-09-11 | Simulateur visiteur : GitHub Pages **et** Streamlit | Pages seul ; Streamlit seul | Le recruteur joue sans installer ; la démo locale reste. Pédagogie RC / déphasage / \(Z\) fitté dans cette étape, pas avant que \(J\) existe. |
| 2026-09-11 | Labo stratégies = Streamlit dynamique ; Pages = leçon RC. Burn-in 24 h d'hystérésis, météo multi-jours, scores hors transitoire | Arène JSON 48 h, \(x_0=(18,16)\), sinus répété | Un départ gelé et un axe « heures depuis 18 h » rendent le MPC magique. Le QP ne tourne pas dans le navigateur. |
| 2026-09-11 | Heures sous T_conf si écart > 0,1 °C ; T_sp rafraîchi chaque pas pour réaliser le P du bloc | Compter tout écart ; T_sp figé 30 min | Le capteur ne voit pas 0,05 °C. Une consigne figée coupe le chauffage dès que l'air monte dans la bande n. |
| 2026-09-12 | Split PEM chronologique, pas de semaines mélangées ni RC ouvré / week-end | Shuffle de semaines ; deux modèles selon le calendrier | Le Kalman identifie une trajectoire, pas des lignes iid. \(R\), \(C\) ne changent pas le samedi : l'occupation va dans \(u\) ou \(w\), pas dans un second bâtiment. Détail : `docs/identification-split.md`. |
